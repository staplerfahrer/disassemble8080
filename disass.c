...
int disassemble8080(unsigned char * disassembled, 
    unsigned char * romBytes, int bufferSize)
{
    int sizePerOpcode = 18;

    int pc = 0;

    while (!eof(bufferSize, pc))
    {
        pc += disassemble8080Op(romBytes, pc);
    }
